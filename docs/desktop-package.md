...
- WebUI 非桌面运行时的 raw `.env` 备份/恢复属于高风险 Web 管理操作，必须先通过微信开放平台 OAuth 建立受保护 Web 会话，并满足该操作的独立、明确权限策略；`rbac.manage` 或 `admin` 角色本身不会自动授予此能力
...
- 桌面端仍可直接使用配置备份/恢复能力；普通 Web 会话、小程序 Bearer 或旧密码管理员登录都不能绕过上述 Web 管理边界。
...
- Windows 防火墙或服务器安全组仍需放行实际监听端口；如需对外暴露，必须通过 HTTPS、微信开放平台 OAuth 和精确 `CORS_ORIGINS` 保护 Web 身份边界
