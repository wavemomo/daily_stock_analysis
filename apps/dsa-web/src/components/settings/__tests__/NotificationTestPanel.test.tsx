...
describe('NotificationTestPanel', () => {
  ...
  it('uses translated defaults when UI language changes and user has not edited fields', async () => {
    ...
    expect(titleInput).toHaveValue('万股图录 通知测试');
    expect(contentInput).toHaveValue('这是一条来自万股图录 Web 设置页的通知测试消息。');

    fireEvent.click(screen.getByRole('button', { name: 'switch-en' }));

    await waitFor(() => {
      expect(titleInput).toHaveValue('upupup notification test');
      expect(contentInput).toHaveValue('This is a test notification from the upupup Web settings page.');
    });

    fireEvent.click(screen.getByRole('button', { name: /发送测试|Send test/ }));
    await waitFor(() => expect(testNotificationChannel).toHaveBeenCalledWith(expect.objectContaining({
      title: 'upupup notification test',
      content: 'This is a test notification from the upupup Web settings page.',
      timeoutSeconds: 20,
    })));
  });
