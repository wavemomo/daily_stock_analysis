...
const settingsHelpZhCN: SettingsHelpMap = {
  'settings.ai_model.GENERATION_BACKEND': {
    ...
    valueNotes: [
      ...
      'Docker、云服务器、CI 不天然拥有你本机的登录状态；万股图录不读取 Codex/Claude/OpenCode 登录凭据文件，但对应 CLI 自己可能使用它的登录状态。',
    ],
  ...
  'settings.ai_model.OPENCODE_CLI_MODEL': {
    ...
    showFieldKey: true,
    summary: '可选：指定万股图录调用 OpenCode run 时传给 --model 的模型名。',
    usage: '仅在“分析生成方式”选择 OpenCode CLI 时生效。留空时万股图录不传 --model，使用你本机 OpenCode 的默认模型配置。',
    valueNotes: [
  ...
  'settings.data_source.FUTU_OPEND_PORT': {
    ...
    impact: ['影响万股图录与 Futu OpenD 的连接。'],
    notes: ['修改后通常需要重启万股图录服务以重新建立连接。'],
  },
  ...
  'settings.agent.AGENT_BACKEND': {
    title: '问股生成方式',
    showFieldKey: false,
    summary: '选择问股 Chat 使用默认模型配置，还是调用运行万股图录设备上的 Codex。',
    usage: '通常保持“自动（推荐）”。自动模式不会启用实验性的 Codex；只有确认运行万股图录的设备已安装并登录 Codex 后，才选择 Codex 本地 Agent。',
    valueNotes: [
      '“自动（推荐）”与“默认模型配置”都继续使用现有模型和 API 配置。',
      '“Codex 本地 Agent（实验）”目前只支持单 Agent 问股，不支持 Codex Multi Agent 或 Codex Deep Research。',
      'Codex 本地 Agent 当前支持 macOS、Linux，以及完整运行于 WSL 的万股图录后端；暂不支持原生 Windows 后端。',
      '本地 Agent 不等于离线模型；股票问题和工具结果可能由 Codex 自身配置的服务处理。',
    ],
    impact: ['只影响问股 Chat，不改变普通报告、定时分析、现有 Multi Agent 或 Deep Research。'],
    notes: [
      '万股图录不读取或保存 Codex 登录凭据，Codex 进程使用自己的登录状态。',
...
const settingsHelpEnUS: SettingsHelpMap = {
  'settings.ai_model.GENERATION_BACKEND': {
    ...
    valueNotes: [
      ...
      'Docker, cloud servers, and CI do not automatically have your local login state. upupup does not read Codex/Claude/OpenCode credential files, but the corresponding CLI itself may use its login state.',
    ],
  ...
  'settings.ai_model.OPENCODE_CLI_MODEL': {
    ...
    usage: 'Only applies when Analysis Generation Method is OpenCode CLI. Leave it empty and upupup will not pass --model, so OpenCode uses its local default model configuration.',
    valueNotes: [
  ...
  'settings.data_source.FUTU_OPEND_HOST': {
    ...
    valueNotes: ['The OpenD service must be reachable from the upupup container.'],
  ...
  'settings.data_source.FUTU_OPEND_PORT': {
    ...
    impact: ['Affects the upupup connection to Futu OpenD.'],
    notes: ['Restarting upupup is normally required after changing it.'],
  },
  'settings.data_source.FUTU_HK_REALTIME_SOURCE_PRIORITY': {
  ...
  'settings.agent.AGENT_BACKEND': {
    ...
    showFieldKey: false,
    summary: 'Choose whether ask-stock Chat uses the default model configuration or Codex on the device running upupup.',
    usage: 'Keep Auto (recommended) unless Codex is installed and signed in on the device running upupup. Auto never enables the experimental Codex route.',
    valueNotes: [
      ...
      'Codex local Agent currently supports macOS, Linux, and an upupup backend running completely inside WSL; native Windows backends are not supported yet.',
    ...
    notes: [
      'upupup does not read or store Codex credentials. The Codex process uses its own sign-in state.',
