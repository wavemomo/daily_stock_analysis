...
- **AI 建议**（决策信号）：两端统一使用「AI 建议」这一名称，入口分别是 Web 侧栏与小程序「我的」菜单及个股详情页。小程序页与 Web 功能对齐：
  - **完整过滤器**：信号列表支持按状态、股票代码、市场、操作、市场阶段、来源类型、来源报告 ID 组合筛选（`GET /decision-signals` 的 `market`/`action`/`market_phase`/`source_type`/`source_report_id`）；填报告 ID 时来源自动收敛为 `analysis`。
  - **最新建议**：输入股票代码后拉取该股票最新 active 建议（`GET /decision-signals/latest/{code}`，携带 `market`/`limit`），点击卡片可进入详情。
  - **决策风格校准**：结果统计页解析 `outcomes/stats` 返回的 `profile_calibration`，按决策风格（保守/均衡/激进）展示命中率、平均收益、未命中率、无法评估率、最大不利波动五项指标，并可在「按周期 / 按操作」两种细分间切换；样本不足按 `minimum_completed_sample_size` 阈值提示。
  - **两步式重评估**：先以 `persist=false` 预览动作、评分、置信度、入场/止损/目标、风控 raw→final 调整与提示，仅当风控通过时才出现「写入信号」按钮以 `persist=true` 落库。
  - **时间线**：新增「时间线」页，按股票代码 + 时间范围（30/90/180 天）+ 状态 + 决策风格查询，用 `created_from`/`created_to` 拉取并按时间排序展示。
