...
- [修复] 小程序补齐「首次启动配置检查」入口：后端接口与客户端封装早已存在但无任何调用，现已在系统设置页顶部展示与 Web 同源的检查结果（模型、问股模型、自选股、通知、存储），支持手动刷新；接口失败时降级提示，不阻断设置页其余内容。
- [修复] 小程序系统设置的「选股」开关不再显示后端英文原文：补齐中文标题与描述（含默认值口径与风险提示），与 Web 端表述一致。
- [修复] 纠正选股开关的默认值文案：实际默认为开启（后端默认值、配置解析与 `.env.example` 三处一致），但 Web 帮助文案写成「默认关闭」，已改为「默认开启。设为 false 后隐藏选股入口并停用选股策略。」
- [改进] 小程序「决策信号」统一改称「AI 建议」，与 Web 端命名一致（菜单、页面标题、空态、新建入口、个股详情卡片与功能额度名称）。
- [修复] Token 用量页在按用户数据缺失字段时不再整页白屏。
- [文档] `docs/miniapp.md` 补充按用户用量契约与 `scope` 字段说明，并新增两端能力一致性小节（首次启动配置检查、智能导入归属、AI 建议命名）。
- [测试] 新增按用户用量聚合、作用域标识、智能导入写入个人自选的回归测试，并同步受影响的既有测试。
...
- [改进] 移除 Web 端微信开放平台扫码 OAuth 登录，Web 统一使用邮箱密码登录。删除 `/api/v1/web-auth/wechat/start` 与 `/api/v1/web-auth/wechat/callback` 端点、`WechatOpenWebAuthService`、`web_wechat_login_transactions` 表模型及 `WECHAT_OPEN_WEB_APP_ID`/`WECHAT_OPEN_WEB_APP_SECRET`/`WECHAT_OPEN_WEB_REDIRECT_URI` 配置；小程序 `wx.login` 登录与显式身份绑定（identity-bind，仍复用 `WECHAT_OPEN_WEB_STATE_TTL_SECONDS`）不受影响。
