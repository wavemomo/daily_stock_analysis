...
#### Web / Mini-program Feature Parity

- Ordinary Web users can open the standalone `/feature-quotas` page from the sidebar to view their server-calculated feature quotas. The page is read-only. Administrators continue to manage global policies, plans, per-user plans, single-feature overrides, and whitelists in the Web `Access Control` page rather than through a duplicate management surface.
- The mini program stores its own page-language preference under `upupup.language` and supports `zh-CN` / `en-US`. It does not change Web `dsa.uiLanguage` or report/Ask Stock `REPORT_LANGUAGE`. Every page registered in `app.json` shares this state, refreshes immediately when it changes, and resynchronizes when the page becomes visible; the Mine header separates brand information, account status, and language actions so the language control remains intact on narrow screens.
- The mini program translates deterministic UI copy, labels, and states only. Server-authored free text, Markdown reports, Agent replies, and user input remain unchanged; switching UI language does not rewrite the existing `report_language: 'zh'` API contract.
- Web `/decision-signals` and mini-program `pages/decision-signals/index` use the same decision-signal API. Both detail views present the same-source `operation_advice` as “operation advice”, instead of generating separate AI recommendation text in each client.

#### Mini-program System Settings and Web Management Boundary

The mini-program settings page uses the strictly Bearer-only `/api/v1/miniapp/system/*` surface. The caller first obtains a Bearer for the same canonical user through mini-program WeChat login, and the server applies the relevant RBAC check; this surface provides masked configuration reads, schema and setup/backend/scheduler status, configuration validation, and version-protected non-raw updates only. Raw `.env` import/export, immediate scheduler runs, generation-backend smoke tests, external LLM/notification channel tests, and model discovery are high-risk Web management operations. They are available only through the protected Web session created by WeChat Open Platform OAuth at `/api/v1/system/*` and are governed by an independent, explicit authorization policy; having `rbac.manage` or the `admin` role alone does not automatically expand these capabilities.

The Web sidebar uses `POST /api/v1/web-auth/logout`: the request must include the current `dsa_user_session` Cookie, its session-bound `X-CSRF-Token`, and an exactly trusted `Origin`. On success it explicitly navigates to the login page; a `401` means the session is already expired or revoked, so the client still clears local state and navigates. A `403 csrf_failed` keeps the current session view and asks the user to refresh and retry rather than hiding the server-side session state. Other failures also keep the current view and show a retryable error, preventing a network or revocation failure from appearing as a successful logout. Mini-program Bearer logout and revocation use their Bearer-only endpoint and can never act as a browser Cookie or cross-client credential.
...
| `WECHAT_OPEN_WEB_APP_ID` | WeChat Open Platform website-application AppID used only by the backend for Web QR OAuth. | - |
| `WECHAT_OPEN_WEB_APP_SECRET` | WeChat Open Platform website-application AppSecret; keep it in the backend deployment environment only. Never expose it to Web builds, clients, logs, or errors. | - |
| `WECHAT_OPEN_WEB_REDIRECT_URI` | Public HTTPS callback registered with WeChat Open Platform, for example `https://example.com/api/v1/web-auth/wechat/callback`. | - |
| `WECHAT_OPEN_WEB_STATE_TTL_SECONDS` | Lifetime in seconds for a Web OAuth state/binding transaction. | `300` |
| `WEB_USER_SESSION_TTL_SECONDS` | Lifetime in seconds for the HttpOnly `dsa_user_session` Cookie established by Web WeChat OAuth; it never reuses a mini-program Bearer token. | `604800` |
| `WECHAT_MINIAPP_APP_ID` | Mini-program AppID used by backend `code2session`. | - |
| `WECHAT_MINIAPP_APP_SECRET` | Mini-program AppSecret; keep it in the backend deployment environment only. | - |
| `WECHAT_MINIAPP_CODE2SESSION_TIMEOUT_SECONDS` | Mini-program `code2session` timeout in seconds. | `8` |
| `WECHAT_MINIAPP_SESSION_TTL_HOURS` | Mini-program Bearer-session lifetime in hours. | `720` |
| `CORS_ORIGINS` | Explicit browser API origins. It does not replace OAuth callback registration, and `*` is not a trusted unsafe-write Origin. | - |
...
### Unified Web / Mini-program Identity, RBAC, and CSRF

The Web and mini program use the same canonical user. Web sign-in is completed by WeChat Open Platform OAuth: the browser begins at `GET /api/v1/web-auth/wechat/start`, and `GET /api/v1/web-auth/wechat/callback?code=&state=` establishes the HttpOnly `dsa_user_session` before redirecting to `/`. The Web reads the current user and session-bound `csrf_token` from `GET /api/v1/web-auth/me`. An existing identity can be deliberately linked only through `POST /api/v1/web-auth/identity-bind/start|consume`; the old Web login link, scan approval, and password administrator login no longer exist. The mini program sends `{"code":"<wx.login code>"}` to `POST /api/v1/miniapp/auth/login` to obtain a Bearer credential, then accesses the same principal through Bearer-only `/api/v1/miniapp/*` endpoints. A Bearer credential never enters browser Cookies, URLs, localStorage, logs, or Web requests.

A protected common `/api/v1/*` request must use exactly one valid credential type: a valid Web Cookie together with a Bearer credential always returns `400 authentication_conflict`. Both `/api/v1/rbac/*` and `/api/v1/miniapp/rbac/*` require `rbac.manage`; the latter remains strictly Bearer-only, while the former resolves the principal through its current entry-point credential. `admin` is an RBAC role only and cannot bypass an `owner_user_id == principal.user_id` resource scope. High-risk system actions require their own explicit authorization policy; never infer a permission code from a role name. The system does not support service principals, API keys, or bot tokens as DSA credentials, and `XAI_API_KEY` and LLM provider keys are not user credentials.

Every unsafe Cookie-authenticated browser request (`POST`, `PUT`, `PATCH`, `DELETE`) must include its session-bound `X-CSRF-Token`, and its `Origin` must exactly match a default local-development origin or an entry in `CORS_ORIGINS`; `POST /api/v1/web-auth/logout` follows the same rule. `*` is not a trusted Origin, so a deployed custom Web domain must be added as a complete explicit origin in `CORS_ORIGINS`. Bearer-only mini-program requests do not imitate or depend on the Cookie/CSRF flow. Feature entitlement is derived only from `user_override`, `whitelist_feature`, `whitelist_all`, `plan`, or `global_policy`; unlimited quota comes only from a whitelist, never automatically from the `admin` role or Cookie type. Screening reserves quota before background-task admission and releases the original UTC `period_start` if queue submission fails; release failure is logged without masking the original queue error.
